	...
	def forward(self, x1, x2, theta):
		...
		for i, output in enumerate(encoder_outs_1):
			...
			if(self.geometric_model == "affine"):
				grid_gen = geometric_transformer.AffineGridGen(out_h=size[3], out_w=size[3])
			else:
				grid_gen = geometric_transformer.TpsGridGen(out_h=size[3], out_w=size[3], grid_size=config.TPS_GRID_SIZE)
